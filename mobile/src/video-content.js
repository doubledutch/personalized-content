...
export default class VideoContent extends Component {
  ...
  render() {
    ...
    return (
      <View style={[s.container]}>
        <View style={s.dimensionStyle}>{this.renderPlayer(url)}</View>
  ...
  renderVimeoPlayer(url) {
    return <WebView source={url} style={s.video} />
  }
  ...
  renderVideoPlayer(url) {
    return (
      <TouchableHighlight
        ref={ref => {}}
        style={{ flex: 1 }}
        onPress={() => this.videoRef.presentFullscreenPlayer()}
      >
        <Video ref={ref => {}} source={{ uri: url }} style={s.video} paused />
      </TouchableHighlight>
    )
  }
  ...
  renderPlayer(video) {
    if (video) {
      ...
      if (video.toLowerCase().indexOf('vimeo.com') >= 0) {
